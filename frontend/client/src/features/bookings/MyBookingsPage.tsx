...
import { IndianRupee, Users, ArrowRight, MapPin, XCircle, Loader2 } from "lucide-react";
  ...
  if (isLoading) {
    return (
      <div className="grid gap-2">
        {[...Array(3)].map((_, i) => (
          <div key={i} className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 animate-pulse h-24" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {list.map((b) => {
        const t = b.trip;
        const title = t
          ? `${t.origin?.name || "Origin"} → ${t.destination?.name || "Destination"}`
          : `Trip ${String(b.tripId).slice(-6)}`;
        const when = t?.startTime ? fmtDateTime(t.startTime) : "";

        const canCancel = b.status === "pending" && (!t || new Date(t.startTime) > new Date());
        ...
        return (
          ...
          >
            <div className="min-w-0">
              ...
              <Link to={`/bookings/${b._id}`} className="font-semibold text-lg hover:underline break-words">
                {t ? (
              ...
              <div className="text-sm text-slate-500 mt-1 inline-flex items-center gap-3">
                <span className="inline-flex items-center gap-1"><Users size={16} />{b.seats} seat(s)</span>
                {t?.pricePerSeat != null && (
                  <span className="inline-flex items-center gap-1">
                    <IndianRupee size={16} /> {t.pricePerSeat}
                  </span>
                )}
                <span
            ...
            <div className="shrink-0 flex items-center gap-2">
              ...
              >
                {cancelMut.isLoading ? <Loader2 size={16} className="animate-spin" /> : <XCircle size={16} />}
                Cancel
              </button>
            </div>
          </div>
