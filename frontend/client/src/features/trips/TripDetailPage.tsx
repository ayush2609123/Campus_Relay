// client/src/features/trips/TripDetailPage.tsx
...
export default function TripDetailPage() {
  ...
  const isOwner = Boolean(me?._id && t?.driverId && String(me._id) === String((t as any).driverId));

  const total = useMemo(() => {
    if (!t?.pricePerSeat) return undefined;
    return Number(t.pricePerSeat) * Number(seats || 0);
  }, [t?.pricePerSeat, seats]);

  const inc = () => setSeats((s) => Math.min(t?.seatsLeft ?? 1, s + 1));
  const dec = () => setSeats((s) => Math.max(1, s - 1));
  const onInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    const v = Math.floor(Number(e.target.value || 1));
    if (Number.isNaN(v)) return;
    const max = t?.seatsLeft ?? 1;
    setSeats(Math.max(1, Math.min(max, v)));
  };
  ...
  const onBook = async () => {
    ...
    try {
      setCreating(true);
      const res = await createBooking({ tripId: t._id, seats });
  ...
  if (error || !t) {
    return (
      <div className="p-6">
        <Link to="/find-trips" className="inline-flex items-center gap-1 text-brand-700 dark:text-brand-300 mb-4">
  ...
  const bookDisabled = isOwner || t.status !== "published" || (t.seatsLeft ?? 0) <= 0 || creating;

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <Link to="/find-trips" className="inline-flex items-center gap-1 text-brand-700 dark:text-brand-300">
          <ArrowLeft size={16} /> Back
        </Link>
      ...
      <div className="grid lg:grid-cols-[1fr,320px] gap-4">
        ...
        <aside className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 h-fit">
          {isOwner ? (
            <>
              <div className="rounded-xl border border-amber-300 bg-amber-50 dark:border-amber-900 dark:bg-amber-950 p-3 text-sm mb-2">
                You are the driver of this trip. Manage it from the driver panel.
              </div>
              <Link
                to={`/driver/trips/${t._id}`}
                className="w-full inline-flex justify-center rounded-xl bg-slate-200 dark:bg-slate-800 py-2 hover:opacity-90"
              >
                Manage this trip
              </Link>
            </>
            ...
            <>
              <div className="text-sm text-slate-500 mb-2">Ready to go?</div>
              <div className="mb-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={dec}
                    className="h-10 w-10 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-900"
                  >−</button>
                  <input
                    type="number"
                    min={1}
                    ...
                    value={seats}
                    onChange={onInput}
                    className="h-10 w-16 text-center rounded-xl border border-slate-200 dark:border-slate-800 bg-transparent"
                  />
                  <button
                    type="button"
                    onClick={inc}
                ...
                <div className="mt-2 text-sm text-slate-600 dark:text-slate-300">
                  ₹ {t.pricePerSeat} / seat
                  {typeof total === "number" && <span className="ml-2"> · <strong>Total: ₹ {total}</strong></span>}
                </div>
              </div>
              ...
              <button
                ...
                onClick={onBook}
              >
                {creating ? "Creating booking…" : (t.seatsLeft ?? 0) > 0 ? "Book this trip" : "Sold out"}
              </button>
